...
export default function Navbar() {
  ...
  const navList = (
    <ul className="flex text-c-body-fade justify flex-col md:flex-row md:justify-center gap-4 md:gap-0 md:[&>li>a]:px-6 [&>li:hover]:text-white font-medium [&>li>a]:transition-all">
      ...
      <li>
        <Link
          href="/projects"
          className={isLinkActive("/projects") ? "text-white" : ""}>
          Projects
        </Link>
      </li>
      <li>
        <Link
          href="/blog"
          className={isLinkActive("/blog") ? "text-white" : ""}>
          Blog
        </Link>
      </li>
      <li>
        <Link
          href="/contact"
          className={isLinkActive("/contact") ? "text-white" : ""}>
          Contact
        </Link>
      </li>
    </ul>
  ...
  return (
    <>
      ...
      <nav className="hidden md:block -mt-10 mx-auto text-c-body-fade sticky top-6 bg-c-black-grey py-2.5 px-6 rounded-full drop-shadow-lg w-[420px] z-10 transition-all duration-300">
        {navList}
      </nav>
      <nav
        data-navdisapear
        className="block md:hidden z-20 transition-all duration-[650ms] sticky top-4 ">
        <div className="flex items-center justify-between mt-4 mx-6 text-c-body-fade  bg-c-black-grey py-2 pl-6 pr-4 rounded-full drop-shadow-md">
          ...
          <button
            ...
            className={`text-white text-2xl bg-c-secondary p-1.5 rounded-full hover:opacity-80 ${
              isNavListVisible ? "-rotate-180" : ""
        ...
        <div
          ...
          className={`${
            isNavListVisible
              ...
              : "-translate-y-12 -z-10 opacity-0"
          }  absolute text-sm w-[150px] mr-9 mt-4 rounded-xl right-0 bg-c-black-grey shadow-lg px-8 py-8 transform transition-all duration-300`}>
          {navList}
          <div className="text-c-body-fade hover:text-white mt-5 flex justify-start items-center">
            {sosmedList}
          </div>
        </div>
      </nav>
